...
CRC24_TABLE = (0x000000, 0x864cfb, 0x8ad50d, 0x0c99f6, 0x93e6e1, 0x15aa1a, 0x1933ec,
...
def get_crc24(data: str) -> dict:
    ...
    crc = 0x00b704ce
    for byte in data:
        crc = (CRC24_TABLE[((crc >> 16) ^ byte) & 0xff] ^ (crc << 8)) & 0x00ffffff
    return {"CRC24_dec": crc, "CRC24_hex": hex(crc)}
